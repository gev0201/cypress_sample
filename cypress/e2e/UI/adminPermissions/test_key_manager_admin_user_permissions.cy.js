...
describe('test suite implemented to test key manager admin user permissions', () => {
    let userData, links;
    before('login and create admin user with only key manager role', () => {
        cy.fixture('AdminUsers/KeyManager').then((fixtureData) => {
    ...
    beforeEach('login with Key Manager user', () => {
        cy.login(userData.username, userData.password);
    });

    it('test case implemented to login as admin user with only key manager role and verify correct menu items are displayed', () => {
