...
describe('test suite implemented to test job manager admin user permissions', () => {
    let userData, links;
    before('login and create admin user with only job manager role', () => {
        cy.login(Cypress.env('adminUserName'), Cypress.env('adminPassword'));
        cy.fixture('AdminUsers/JobManager').then((fixtureData) => {
    ...
    beforeEach('login with Job Manager user', () => {
        cy.login(userData.username, userData.password);
    });

    it('test case implemented to login as admin user with only job manager role and verify correct menu items are displayed', () => {
