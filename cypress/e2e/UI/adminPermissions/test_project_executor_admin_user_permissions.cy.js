...
describe('test suite implemented to test project executor admin user permissions', () => {
    let userData, links;
    before('login and create admin user with only project executor role', () => {
        cy.login(Cypress.env('adminUserName'), Cypress.env('adminPassword'));
        cy.fixture('AdminUsers/ProjectExecutor').then((fixtureData) => {
    ...
    beforeEach('login with Project Executor user', () => {
        cy.login(userData.username, userData.password);
    });

    it('test case implemented to login as admin user with only project executor role and verify correct menu items are displayed', () => {
