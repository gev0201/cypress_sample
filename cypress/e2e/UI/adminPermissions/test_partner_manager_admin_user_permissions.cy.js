...
describe('test suite implemented to test partner manager admin user permissions', () => {
    let userData, links;
    before('login and create admin user with only partner manager role', () => {
        cy.login(Cypress.env('adminUserName'), Cypress.env('adminPassword'));
        cy.fixture('AdminUsers/PartnerManager').then((fixtureData) => {
    ...
    beforeEach('login with Partner Manager user', () => {
        cy.login(userData.username, userData.password);
    });

    it('test case implemented to login as admin user with only partner manager role and verify correct menu items are displayed', () => {
